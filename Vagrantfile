...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "patchwork" do |box|
    box.vm.box = "ubuntu/focal64"
    box.vm.box_version = "20220511.0.0"
    box.vm.hostname = 'patchwork.vagrant.example.lan'
    ...
    box.vm.provision "shell",
      ...
      env: {  'FACTER_my_host': 'patchwork.vagrant.example.lan',
              'FACTER_my_ip': '192.168.59.80' }
    box.vm.provision "shell",
      inline: "/opt/puppetlabs/bin/puppet apply /vagrant/vagrant/patchwork.pp --modulepath=/vagrant/modules"
  end
end
